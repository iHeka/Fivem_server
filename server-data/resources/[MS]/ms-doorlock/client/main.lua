...
Citizen.CreateThread(function()
	while true do
		for key, doorID in ipairs(ms.Doors) do
...
Citizen.CreateThread(function()
	while true do
		...
		for k,doorID in ipairs(ms.Doors) do
			local distance
			...
			if distance < maxDistance then
				...
				if isAuthorized then
					if doorID.locked then
						displayText = "~g~E~w~ - Ouvrir"
					elseif not doorID.locked then
						displayText = "~g~E~w~ - Fermer"
					end
				...
				if doorID.locking then
					if doorID.locked then
						displayText = "Ouveture.."
					else
						displayText = "Fermeture.."
					end
				end
...
AddEventHandler('lockpicks:UseLockpick', function()
	local ped = GetPlayerPed(-1)
	local pos = GetEntityCoords(ped)
	MSCore.Functions.TriggerCallback('ms-doorlock:server:GetItem', function(hasItem)
		for k, v in pairs(ms.Doors) do
			local dist = GetDistanceBetweenCoords(pos, ms.Doors[k].textCoords.x, ms.Doors[k].textCoords.y, ms.Doors[k].textCoords.z)
			if dist < 1.5 then
				if ms.Doors[k].pickable then
					if ms.Doors[k].locked then
						if hasItem then
							closestDoorKey, closestDoorValue = k, v
							TriggerEvent('ms-lockpick:client:openLockpick', lockpickFinish)
						else
							MSCore.Functions.Notify("Il me faut des outils...", "error")
						end
					else
						MSCore.Functions.Notify('La porte est déjà ouverte ???', 'error', 2500)
					end
				else
					MSCore.Functions.Notify('c\'est vraiment solide !', 'error', 2500)
				end
			end
		end
    end, "screwdriverset")
...
function lockpickFinish(success)
    if success then
		MSCore.Functions.Notify('C\'est ouvert!', 'success', 2500)
		setDoorLocking(closestDoorValue, closestDoorKey)
    else
        MSCore.Functions.Notify('Echec..', 'error', 2500)
    end
end
...
function setDoorLocking(doorId, key)
    ...
    SetTimeout(400, function()
		...
		TriggerServerEvent('ms-doorlock:server:updateState', key, doorId.locked)
	end)
end
...
RegisterNetEvent('ms-doorlock:client:setState')
AddEventHandler('ms-doorlock:client:setState', function(doorID, state)
	ms.Doors[doorID].locked = state
end)

RegisterNetEvent('ms-doorlock:client:setDoors')
AddEventHandler('ms-doorlock:client:setDoors', function(doorList)
	ms.Doors = doorList
end)

RegisterNetEvent('MSCore:Client:OnPlayerLoaded')
AddEventHandler('MSCore:Client:OnPlayerLoaded', function()
    TriggerServerEvent("ms-doorlock:server:setupDoors")
end)